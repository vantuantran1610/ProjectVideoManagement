...
public class AbstractDao<T> {
	...
	public List<T> findAll(Class<T> clazz, boolean exitstIsActive) {
		String entityName = clazz.getSimpleName();
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT o FROM ").append(entityName).append(" o");
		if (exitstIsActive == true) {
			sql.append(" WHERE isActive = 1");
		}

		TypedQuery<T> query = entityManager.createQuery(sql.toString(), clazz);
		return query.getResultList();
		// SELECT 0 FROM entity o WHERE isActive = 1;
	}

	public List<T> findAll(Class<T> clazz, boolean exitstIsActive, int pageNumber, int pageSize) {
		String entityName = clazz.getSimpleName();
		StringBuilder sql = new StringBuilder();
		sql.append("SELECT o FROM ").append(entityName).append(" o");
		if (exitstIsActive == true) {
		...
		TypedQuery<T> query = entityManager.createQuery(sql.toString(), clazz);
		query.setFirstResult((pageNumber - 1) * pageSize);// danh index tu 0
		query.setMaxResults(pageSize);
		return query.getResultList();

	}

	// Select o form user o where o.username = ?0 and o.password = ?1;
