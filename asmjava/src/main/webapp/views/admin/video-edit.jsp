...
<body class="hold-transition sidebar-mini layout-fixed">
  ...
  $.widget.bridge('uibutton', $.ui.button)
	...
	<script>
		var titleOrigin, hrefOrigin, descriptionOrigin, posterOgirin;
		$(document).ready(function(){
			titleOrigin = $('#title').val();
			hrefOrigin = $('#href').val();
			descriptionOrigin = $('#description').val();
			posterOgirin = $('#poster').val();
		});
		...
		$('#resetBtn').click(function(){
			 ...
			 $('#poster').val(posterOgirin);
			 if(posterOgirin.length > 0){
				 $('#imgPreview').attr('src', posterOgirin);
			 }
		});
		...
		$('#submitBtn').click(function(){
			...
			if(isEdit == 'true') {
				url : '/asmjava/admin/video?action=edit&href=' + hrefOrigin;
			}else{
				url : '/asmjava/admin/video?action=add';
			}
			var formData = {
		...
		$.ajax({
			...
			window.location.href = 'http://localhost:8080/asmjava/admin/video?action=view';
		}).fail(function(error){
